...
public:
    ...
    bool pop(size_t& element) {
        unique_lock<mutex> guard(lock);
        while (elements.empty() && !closed) {
            not_empty.wait(guard);
        }
        if (elements.empty() && closed) {
            return false;
        }
...
void process_one_location(size_t loc_i, const vector<string>& location_names, const LocationHours& loc_hours, const vector<Agent>& agents, const unordered_map<int, size_t>& id_to_index, const Params& params, vector<double>& shared_no_infection, vector<mutex>& agent_locks) {
    ...
    for (const auto& item : local_no_infection_factor) {
        ...
        lock_guard<mutex> guard(agent_locks[agent_idx]);
        shared_no_infection[agent_idx] *= factor;
    }
}

void worker_loop(LocationTaskQueue& tasks, const vector<string>& location_names, const LocationHours& loc_hours, const vector<Agent>& agents, const unordered_map<int, size_t>& id_to_index, const Params& params, vector<double>& shared_no_infection, vector<mutex>& agent_locks, atomic<size_t>& processed_locations) {
    size_t loc_i;
    while (tasks.pop(loc_i)) {
        process_one_location(loc_i, location_names, loc_hours, agents, id_to_index, params, shared_no_infection, agent_locks);
...
pair<vector<int>, vector<double>> simulate_exposures_concurrent_by_location(const vector<Agent>& agents, const Params& params, mt19937& rng, const LocationHours& loc_hours, const vector<string>& location_names, const unordered_map<int, size_t>& id_to_index) {
    ...
    tasks.close();
    vector<double> daily_no_infection(agents.size(), 1.0);
    vector<mutex> agent_locks(agents.size());
    atomic<size_t> processed_locations(0);
    vector<thread> workers;
    workers.reserve(worker_count);

    for (unsigned int w = 0; w < worker_count; ++w) {
        workers.emplace_back(worker_loop, ref(tasks), cref(location_names), cref(loc_hours), cref(agents), cref(id_to_index), cref(params), ref(daily_no_infection), ref(agent_locks), ref(processed_locations));
    }
    ...
    for (size_t i = 0; i < agents.size(); ++i) {
        double p = 1.0 - daily_no_infection[i];
...
struct CliArgs {
    ...
    int days = 30;
    string output_dir = "concurrent_results";
    unsigned int workers = 0;
...
int main(int argc, char* argv[]) {
    try {
        ...
        cout << "\nC++ concurrent simulation complete.\n";
